...
class AE_Model(nn.Module):
    ...
    def initialize(self, opt):
        ...
        self.net_encoder = networks.define_part_encoder(model=self.model_partial_name, input_nc=opt.input_nc, norm=opt.norm, latent_dim=opt.latant_dim)
        self.net_decoder = networks.define_part_decoder(model=self.model_partial_name, output_nc=opt.input_nc, norm=opt.norm, latent_dim=opt.latant_dim)
    ...
    def get_inter(self, input_image, nearnN=3, sex=1, w_c=1, random_=-1):
        ...
        b = jt.code([1, nearnN], "int32", [jt.array(feature_list), jt.array(generated_f), list_len],
        ...
        A_m = A_0
        for i in range(1, nearnN): A_m = np.concatenate((A_m, [feature_list[idx_sort[i], :]]), axis=0)
