...
class Combine_Model(nn.Module):
    ...
    def initialize(self, opt):
        ...
        ##### define networks        
        # generator network
        self.part = {'': (0, 0, 512), 'eye1': (108, 156, 128), 'eye2': (255, 156, 128), 'nose': (182, 232, 160), 'mouth': (169, 301, 192)}

        self.Decoder_Part = {}

        for key in self.part.keys(): self.Decoder_Part[key] = networks.define_feature_decoder(model=key, output_nc=32, norm=opt.norm, latent_dim=opt.latant_dim)

        self.netG = networks.define_G(opt.num_inter_channels, opt.output_nc, opt.ngf, opt.n_downsample_global, opt.n_blocks_global, opt.norm)
    ...
    def load_network(self, network, network_label, epoch_label, save_dir='', save_path=''):
        ...
        if not os.path.isfile(save_path): print('%s not exists yet!' % save_path)
        else: network.load(save_path)
